...
2. On SSTap, add a proxy server as follows. Select `Type` as `SOCKS 5`, `Server IP` as `localhost` or `127.0.0.1`, `Port` fill in the port number entered previously in the `ssh -D` option, and leave other options as is. 
...
3. On SSTap, add a rule as follows. Choose names as you wish, `Proxify DNS request` select `Proxify`, `Action type` as `Proxify`, and in the `Rules list` fill in `192.168.31.0/24`. 
...
- (Optional)Add ip/hostname pairs in `C:\Windows\System32\drivers\etc\hosts` or config a customized DNS service if you know how(more complex but more suitable for larger networks, for simple ones, just modify `hosts` file). Since the normal host resolution inside a LAN is unlikely working this VPN, writing down the hostnames in `hosts` file will simplify your access by typing in hostnames instead of IP addresses. 
